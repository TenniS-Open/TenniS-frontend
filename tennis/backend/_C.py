...
TennisName = "tennis"
TypoTennisName = "TensorStack"
libTennis = find_library(TennisName)

lib = None
if libTennis is not None:
    try:
        lib = CDLL(libTennis, RTLD_GLOBAL)
    except:
        raise
...
if lib is None:
    ...
    lib, msg = libfinder.load_library(TennisName)
...
if lib is None:
    if msg is None:
        raise ImportError("Can find library: {}".format(TennisName))
    else:
        raise ImportError(msg)


def __TS_IMPORT(lib, sym, restype=None, *argtypes):
    # type: (CDLL, str, object, List[object]) -> callable
