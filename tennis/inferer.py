...
def infer_shape_index_patch(node, inputs):
    ...
    x_patch_h = int(origin_patch[0] * height / float(origin[0]) + 0.5)
    x_patch_w = int(origin_patch[1] * width / float(origin[1]) + 0.5)

    return NodeShape((number, channels, x_patch_h, int(landmark // 2), x_patch_w), x.dtype)
...
def infer_nhwc_scale_resize2d(node, inputs):
    ...
    if len(size) == 1:
        ...
        w = y[2]
        if h > w:
            y[1], y[2] = int(size[0] * h / w), size[0]
        else:
            y[1], y[2] = size[0], int(size[0] * w / h)
    elif len(size) == 2:
        y[1], y[2] = size[1], size[0]
    else:
        return None
